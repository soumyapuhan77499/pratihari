...
@section('styles')
    ...
    <style>
        ...
        .filters-card{ border-radius:1rem; border:1px solid var(--border); padding:.75rem; background:#fff; box-shadow:0 6px 18px rgba(2,6,23,.04); }
        .filters-nav .nav-link{ border-radius:.6rem; margin-right:.5rem; padding:.5rem .9rem; font-weight:800; }
        .filters-nav .nav-link.active{ box-shadow:0 8px 24px rgba(124,58,237,.08); }
        .filters-count{ min-width:2.2rem; display:inline-flex; justify-content:center; align-items:center; font-weight:800; border-radius:.5rem; padding:.25rem .45rem; }
        ...
        thead th{position:sticky; top:0; z-index:2;background:#f8fafc; color:var(--ink); font-weight:800;border-bottom:1px solid var(--border);}
        tbody tr:hover{background:#f9fbff;}

        /* Photo */
        .profile-photo{width:56px;height:56px;border-radius:10px;object-fit:cover;transition:transform .25s ease, box-shadow .25s ease;}
        .profile-photo:hover{transform:scale(2.6);box-shadow:0 8px 16px rgba(0,0,0,.25);z-index:5;}

        /* Badges */
        .badge-soft{font-weight:700;border:1px solid var(--border);background:#f8fafc;color:var(--muted);padding:.35rem .6rem;border-radius:.6rem;}
        ...
        @media (max-width:720px){
            .filters-nav{ width:100%; overflow-x:auto; white-space:nowrap; }
            .filters-nav .nav-link{ display:inline-flex; }
        }
    </style>
@endsection
...
<div class="container-fluid my-3">
    ...
    <div class="page-header mb-3">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div class="d-flex align-items-center gap-2">
                <span style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:10px;background:linear-gradient(90deg,var(--brand-a),var(--brand-b));color:#fff;"><i class="fa-solid fa-user-gear"></i></span>
                <div>
    ...
    <div class="filters-section">
        <div class="filters-card d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
            <div class="d-flex align-items-center gap-2 filters-nav" role="tablist" aria-label="Profile status tabs">
                <button class="nav-link active d-flex align-items-center" data-status="all" id="tab-all" type="button" aria-pressed="true" aria-controls="profiles-tbody">
                    <i class="fa-solid fa-list-check me-2"></i> All
                    <span class="filters-count bg-white text-dark ms-2">{{ $profiles->count() }}</span>
                </button>

                <button class="nav-link d-flex align-items-center" data-status="pending" id="tab-pending" type="button" aria-pressed="false" aria-controls="profiles-tbody">
                    <i class="fa-solid fa-clock me-2"></i> Pending
                    <span class="filters-count bg-warning text-dark ms-2">{{ $counts['pending'] }}</span>
                </button>

                <button class="nav-link d-flex align-items-center" data-status="approved" id="tab-approved" type="button" aria-pressed="false" aria-controls="profiles-tbody">
                    <i class="fa-solid fa-check me-2"></i> Approved
                    <span class="filters-count bg-success text-white ms-2">{{ $counts['approved'] }}</span>
                </button>

                <button class="nav-link d-flex align-items-center" data-status="rejected" id="tab-rejected" type="button" aria-pressed="false" aria-controls="profiles-tbody">
            ...
            <div class="d-flex align-items-center gap-2 ms-auto w-100 w-md-auto">
                <div class="input-group" style="min-width:240px;">
                    ...
                    <input id="profile-search" class="form-control form-control-sm" placeholder="Search by name, phone or card..." aria-label="Search profiles">
                    <button id="clear-search" class="btn btn-outline-secondary btn-sm" type="button" title="Clear search" aria-label="Clear search"><i class="fa-solid fa-xmark"></i></button>
                </div>
            </div>
        </div>
    </div>
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody id="profiles-tbody">
                        @foreach ($profiles as $index => $profile)
                            ...
                            <tr data-status="{{ $ps }}" data-search="{{ strtolower($profile->first_name . ' ' . $profile->middle_name . ' ' . $profile->last_name . ' ' . $profile->phone_no . ' ' . $profile->healthcard_no) }}">
                                <td class="fw-bold">{{ $index + 1 }}</td>

                                <td>
                                    <a href="{{ route('admin.viewProfile', $profile->pratihari_id) }}" class="text-decoration-none">
                                        <img src="{{ $profile->photo_url ?? asset($profile->profile_photo ?? 'images/default-avatar.png') }}" class="profile-photo" alt="Profile Photo">
                                    </a>
                                </td>

                                <td>
                                    <a href="{{ route('admin.viewProfile', $profile->pratihari_id) }}" class="btn btn-slim" style="background:var(--amber); color:#1f2937;">
            ...
            $('#profile-search').on('input', function(){
                ...
                $('#profiles-tbody tr').each(function(){
                    ...
                    if (!q) {
                        ...
                        if (active === 'all' || active === rs) $(this).show(); else $(this).hide();
                    } else {
                        if (hay.indexOf(q) !== -1) $(this).show(); else $(this).hide();
                    }
                });
            });
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            error: function(xhr){
                                ...
                                Swal.fire('Error', (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'Could not approve. Please try again.', 'error');
                            }
                        });
                    }
                });
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            error: function(xhr){
                                lockButtons(row,false);
                                Swal.fire('Error', (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'Could not reject. Please try again.', 'error');
                            }
                        });
                    }
                });
