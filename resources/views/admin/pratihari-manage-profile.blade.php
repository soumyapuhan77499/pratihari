            ...
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <div>
                        ...
                        <div class="small opacity-75">Browse, view details, and approve or reject submissions.</div>
                    </div>
                </div>

                <div class="d-flex align-items-center filters-row">
                    <div class="nav nav-pills" role="tablist" aria-label="Profile status tabs">
                        <button class="nav-link active d-flex align-items-center" data-status="all" id="tab-all"
                            type="button">
                            All <span class="badge bg-white text-dark ms-2">{{ $profiles->count() }}</span>
                        </button>
                        <button class="nav-link d-flex align-items-center" data-status="pending" id="tab-pending"
                            type="button">
                            Pending <span class="badge bg-warning text-dark ms-2">{{ $counts['pending'] }}</span>
                        </button>
                        <button class="nav-link d-flex align-items-center" data-status="approved" id="tab-approved"
                            type="button">
                            Approved <span class="badge bg-success text-white ms-2">{{ $counts['approved'] }}</span>
                        </button>
                        <button class="nav-link d-flex align-items-center" data-status="rejected" id="tab-rejected"
                            type="button">
                            Rejected <span class="badge bg-danger text-white ms-2">{{ $counts['rejected'] }}</span>
                        </button>
                    </div>

                    <div class="ms-3">
                        <input id="profile-search" class="form-control form-control-sm search-input"
                                ...
                                <tr data-status="{{ $ps }}"
                                    ...
                                    <td>
                                        <a href="{{ route('admin.viewProfile', $profile->pratihari_id) }}"
                                            ...
                                            <img src="{{ $profile->photo_url ?? asset($profile->profile_photo ?? 'images/default-avatar.png') }}"
                                    ...
                                    <td>
                                        @if ($ps === 'approved')
                                            <span class="status-pill badge-approved status-pill">Approved</span>
                                        @elseif ($ps === 'rejected')
                                            <span class="status-pill badge-rejected status-pill">Rejected</span>
                                        @else
                                            <span class="status-pill badge-pending status-pill">Pending</span>
                                        @endif
                                    </td>
    ...
    <script>
        ...
        $(function() {
            ...
            function showTab(status) {
                ...
                if (status === 'all') $('#tab-all').addClass('active');
                if (status === 'pending') $('#tab-pending').addClass('active');
                if (status === 'approved') $('#tab-approved').addClass('active');
                if (status === 'rejected') $('#tab-rejected').addClass('active');
            }
            ...
            // search box (simple client-side)
            $('#profile-search').on('input', function() {
                const q = $(this).val().trim().toLowerCase();
                $('#profiles-tbody tr:visible').each(function() {
                    const hay = $(this).data('search') || '';
                    if (!q || hay.indexOf(q) !== -1) $(this).show();
                    else $(this).hide();
                });
            });
