                        ...
                        <div class="row g-2 align-items-end">
                            <div class="col-md-3">
                                ...
                                <select name="pratihari_id" id="pratihari_id" class="form-select select2">
                                    ...
                                    @foreach ($pratiharis as $id => $name)
                                        <option value="{{ $id }}"
                                            {{ request('pratihari_id') == $id ? 'selected' : '' }}>{{ $name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="seba_id" id="seba_id" class="form-select select2">
                                    ...
                                    @foreach ($sebas as $id => $name)
                                        <option value="{{ $id }}"
                                            {{ request('seba_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="d-flex gap-2">
                                    <input type="text" name="date_from" id="date_from" class="form-control flatpickr"
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="text" class="form-control" name="search"
                                    placeholder="Pratihari / Beddha IDs / year" value="{{ request('search') }}">
                            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="small fw-bold">
                                                {{ $row->pratihari_name ?? '#' . $row->pratihari_id }}</div>
                                        ...
                                        <td>
                                            @php
                                                $bids = array_filter(
                                                    array_map('trim', explode(',', (string) $row->beddha_id)),
                                                );
                                            @endphp
                                        ...
                                        <td>
                                            <button class="btn btn-sm btn-outline-primary show-transaction"
                                                data-id="{{ $row->id }}">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center small-muted py-3">No transactions found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        $(function() {
            ...
            $(document).on('click', '.show-transaction', function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (!res.transaction) {
                            $('#transactionModalBody').html(
                                '<div class="text-danger py-3 text-center">Not found</div>');
                            return;
                        }
                        ...
                        html +=
                            `<dt class="col-sm-4">Transaction ID</dt><dd class="col-sm-8">${t.id}</dd>`;
                        html +=
                            `<dt class="col-sm-4">Pratihari</dt><dd class="col-sm-8">${t.pratihari_name ?? ('#' + t.pratihari_id)}</dd>`;
                        html +=
                            `<dt class="col-sm-4">Seba</dt><dd class="col-sm-8">${t.seba_name ?? ('#' + t.seba_id)}</dd>`;
                        html +=
                            `<dt class="col-sm-4">Beddha IDs</dt><dd class="col-sm-8">${idsHtml}</dd>`;
                        html +=
                            `<dt class="col-sm-4">Beddha Names</dt><dd class="col-sm-8">${namesHtml}</dd>`;
                        html +=
                            `<dt class="col-sm-4">Year</dt><dd class="col-sm-8">${t.year ?? '—'}</dd>`;
                        html +=
                            `<dt class="col-sm-4">Assigned By (Admin)</dt><dd class="col-sm-8">${t.assigned_by ?? '—'}</dd>`;
                        html +=
                            `<dt class="col-sm-4">Date / Time</dt><dd class="col-sm-8">${t.date_time ?? '—'}</dd>`;
                        html += `</dl>`;
                    ...
                    error: function() {
                        $('#transactionModalBody').html(
                            '<div class="text-danger py-3 text-center">Error loading details.</div>'
                            );
                    }
                });
            });
