...
@section('styles')
    ...
    <style>
        ...
        .profile-hero .avatar{width:84px;height:84px;border-radius:14px;overflow:hidden;border:3px solid rgba(255,255,255,.55);box-shadow:0 10px 22px rgba(2,6,23,.22);flex-shrink:0}
        .profile-hero .avatar img{width:100%;height:100%;object-fit:cover}

        /* Chips */
        .stat-chip{display:flex;align-items:center;gap:.45rem;background:rgba(255,255,255,.15);padding:.35rem .55rem;border-radius:999px;font-weight:600;white-space:nowrap;backdrop-filter:blur(2px)}

        /* Donut rail */
        .donut-card{border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.08);border-radius:14px;box-shadow:inset 0 1px 0 rgba(255,255,255,.15),0 8px 20px rgba(2,6,23,.15);backdrop-filter:blur(3px)}
        .donut-rail{display:flex;flex-wrap:nowrap;gap:12px;overflow-x:auto;padding:6px;scroll-snap-type:x mandatory}
        .donut-rail::-webkit-scrollbar{height:8px}
        .donut-rail::-webkit-scrollbar-thumb{background:rgba(255,255,255,.25);border-radius:8px}
        .donut{flex:0 0 132px;display:flex;flex-direction:column;align-items:center;padding:12px 10px;border-radius:12px;background:var(--soft);scroll-snap-align:start;border:1px solid var(--border)}
        ...
        .tabbar .nav-link{display:flex;align-items:center;gap:.5rem;border:1px solid transparent;background:var(--soft);color:var(--muted);border-radius:11px;padding:.6rem .9rem;font-weight:700;white-space:nowrap;transition:transform .12s,background .2s,color .2s,border-color .2s}
        .tabbar .nav-link:hover{background:#eef2ff;color:var(--ink);transform:translateY(-1px);border-color:rgba(124,58,237,.25)}
        .tabbar .nav-link.active{color:#fff!important;background:linear-gradient(90deg,var(--brand-a),var(--brand-b));border-color:transparent;box-shadow:0 10px 18px rgba(124,58,237,.25)}
        ...
        /* Bheddha pill */
        .beddha-pill{display:inline-flex;align-items:center;gap:.4rem;background:linear-gradient(90deg,var(--brand-a),var(--brand-b));color:#fff;font-weight:700;font-size:.83rem;padding:.35rem .65rem;border-radius:999px;box-shadow:0 6px 14px rgba(124,58,237,.22)}
        ...
        .address-duo .addr-card{border-radius:18px;overflow:hidden;border:1px solid var(--border);background:var(--surface);box-shadow:0 16px 38px rgba(2,6,23,.08)}
        .addr-top{
            position:relative;
            padding:16px;
            color:#fff;
        ...
        .addr-top.perm{background:linear-gradient(120deg,#7c3aed 0%,#ef4444 100%)}
        .addr-top .title{font-weight:900;letter-spacing:.2px;flex:1}
        .addr-top .badge-soft{background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.35);padding:.18rem .5rem;border-radius:999px;font-weight:800}
        .addr-top:after{
        ...
        .addr-row{
            display:grid;grid-template-columns:28px 1fr;gap:.7rem;
            padding:.6rem .2rem;border-radius:10px;position:relative;
        }
        .addr-row + .addr-row{margin-top:.25rem}
        .addr-row:nth-child(odd){background:linear-gradient(180deg,#f8fafc,#ffffff);border:1px dashed rgba(2,6,23,.06)}
        .addr-row:nth-child(even){background:linear-gradient(180deg,#ffffff,#f8fafc);border:1px dashed rgba(2,6,23,.06)}
        .addr-row i{color:#334155;opacity:.95;margin-top:.2rem}
        .addr-row .k{display:block;font-size:.78rem;color:var(--muted)}
        .addr-row .v{font-weight:800;color:var(--ink);word-break:break-word}
        .tagline{display:inline-flex;align-items:center;gap:.4rem;background:#eef2ff;border:1px solid rgba(124,58,237,.25);color:#312e81;font-weight:700;border-radius:999px;padding:.2rem .55rem;font-size:.75rem}
        .addr-foot{display:flex;justify-content:flex-end;gap:.5rem;border-top:1px solid var(--border);padding:10px 16px;background:linear-gradient(180deg,rgba(2,6,23,.02),rgba(2,6,23,.04))}
        .btn-round{border-radius:999px;padding:.38rem .75rem}

        /* Copy tooltip */
        .copy-tip{position:relative}
        .copy-tip[data-copied="1"]::after{
            content:"Copied!";position:absolute;top:-28px;right:0;background:#10b981;color:#fff;font-weight:800;
            padding:.15rem .45rem;border-radius:8px;font-size:.75rem;box-shadow:0 6px 12px rgba(16,185,129,.35)
        }

        /* SEBA cards (unchanged from your last setup) */
        .seba-card{border:1px solid var(--border);border-radius:14px;overflow:hidden;background:var(--surface);box-shadow:0 10px 28px rgba(2,6,23,.06);height:100%}
        .seba-card .seba-head{color:#fff;padding:14px 16px;display:flex;align-items:center;gap:.6rem;font-weight:800}
        ...
        .seba-chip{display:inline-flex;align-items:center;gap:.35rem;padding:.3rem .55rem;border-radius:999px;background:linear-gradient(90deg,var(--brand-a),var(--brand-b));color:#fff;font-weight:700;font-size:.8rem;box-shadow:0 6px 14px rgba(124,58,237,.18)}
        ...
        @media (max-width:576px){.profile-hero{align-items:flex-start}}
    </style>
@endsection
                ...
                <div class="row address-duo g-3 g-lg-4">
                    ...
                    <div class="col-lg-6">
                        <div class="addr-card h-100">
                            ...
                            <div class="addr-foot">
                                <button type="button" class="btn btn-ghost btn-sm btn-round copy-tip" data-clip="{{ trim(($addr->address ?? '').' '.($addr->sahi ?? '').' '.($addr->district ?? '').' '.($addr->state ?? '').' '.($addr->pincode ?? '')) }}">
                    ...
                    <div class="col-lg-6">
                        <div class="addr-card h-100">
                            ...
                            <div class="addr-foot">
                                <button type="button" class="btn btn-ghost btn-sm btn-round copy-tip" data-clip="{{ trim(($addr->per_address ?? '').' '.($addr->per_sahi ?? '').' '.($addr->per_district ?? '').' '.($addr->per_state ?? '').' '.($addr->per_pincode ?? '')) }}">
                                    <i class="fa-regular fa-copy me-1"></i>Copy
                                </button>
...
@section('scripts')
    ...
    <script>
        ...
        const chartColors = {
            profileChart: '#4CAF50', familyChart: '#FF9800', idcardChart: '#2196F3',
            addressChart: '#673AB7', occupationChart: '#009688', sebaChart: '#FF5722', socialmediaChart: '#E91E63',
        };
